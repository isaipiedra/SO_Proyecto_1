...
// INTEGRATED MD5 IMPLEMENTATION (RFC 1321)
...
static void md5_process_block(MD5_Context *ctx, const unsigned char *block) {
    ...
    for (int i = 0; i < 64; i++) {
        ...
        d = c;
        c = b;
        b = b + md5_rotate(a + f + md5_table[i] + x[g], 
                          (i < 16) ? (5 + i%4*4) : ((i < 32) ? (4-i%4) : ((i < 48) ? (i%4*5+4) : ((i-32)%4*7+6))));
        a = temp;
    }
...
static void md5_update(MD5_Context *ctx, const unsigned char *data, unsigned long len) {
    unsigned long i = 0;
    unsigned long index = (ctx->count / 8) % MD5_BLOCK_SIZE;
    
    ctx->count += len * 8;
    unsigned long partLen = MD5_BLOCK_SIZE - index;
...
static void md5_final(unsigned char digest[MD5_DIGEST_SIZE], MD5_Context *ctx) {
...
unsigned long decompress_data(const unsigned char *compressed_data, unsigned long compressed_size,
    ...
    // Caso especial: árbol de un solo nodo (archivo uniforme)
    if (!tree->left && !tree->right) {
    ...
    while (output_position < original_size && bit_position < bit_count) {
        ...
        if (byte_index >= compressed_size) break;  // seguridad
        ...
        if (!current) {
            // Stream inconsistente: abortar limpiamente
            break;
        }
